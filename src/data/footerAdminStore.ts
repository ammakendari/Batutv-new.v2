...
/**
 * Get stored Footer Configuration with fallback to initial config
 */
export function getStoredFooterConfig(): FooterConfig {
  try {
    ...
    console.error('Failed to load footer configuration from localStorage:', err);
    return INITIAL_FOOTER_CONFIG;
  }
}

/**
 * Save Footer Configuration & dispatch notification event to sync all views
 */
export function saveFooterConfig(config: FooterConfig): boolean {
  try {
    ...
    localStorage.setItem(FOOTER_STORAGE_KEY, JSON.stringify(configToSave));

    // Dispatch event for instant reactive update across tabs and components
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent(FOOTER_UPDATED_EVENT, { detail: configToSave })
      );
    }
    return true;
  } catch (err) {
    console.error('Failed to save footer configuration to localStorage:', err);
    return false;
  }
}
...
export function isValidUrlOrPath(val: string): boolean {
  if (!val || val.trim() === '') return true; // empty allowed unless strictly required
...
/**
 * Helper to validate phone number (digits, +, -, spaces, parentheses)
 */
...
export function validateFooterForm(config: FooterConfig): {
  ...
  // 4. Social Media Validation (all optional, but if provided must be valid url)
