...
export const DEFAULT_SECURITY_CONFIG: SecurityConfig = {
  sessionTimeoutMinutes: 120, // 2 Jam
  loginAttemptLimit: 5,
...
export const getStoredSystemInfo = (): SystemInfoConfig => {
  ...
  try {
    ...
    if (!raw) {
      localStorage.setItem(SYSTEM_INFO_STORAGE_KEY, JSON.stringify(DEFAULT_SYSTEM_INFO));
...
export const getStoredMaintenanceConfig = (): MaintenanceConfig => {
  ...
  try {
    ...
    if (!raw) {
      localStorage.setItem(SYSTEM_MAINTENANCE_STORAGE_KEY, JSON.stringify(DEFAULT_MAINTENANCE_CONFIG));
...
export const setStoredMaintenanceConfig = (
  ...
  try {
    ...
    // Update system info status tag too
    setStoredSystemInfo({
...
export const getStoredSecurityConfig = (): SecurityConfig => {
  ...
  try {
    ...
    if (!raw) {
      localStorage.setItem(SYSTEM_SECURITY_STORAGE_KEY, JSON.stringify(DEFAULT_SECURITY_CONFIG));
...
export const getStoredActivityLogs = (): ActivityLogItem[] => {
  ...
  try {
    ...
    if (!raw) {
      localStorage.setItem(SYSTEM_ACTIVITY_LOGS_STORAGE_KEY, JSON.stringify(INITIAL_ACTIVITY_LOGS));
...
export const getStoredCacheStats = (): CacheStats => {
  ...
  try {
    ...
    if (!raw) {
      localStorage.setItem(SYSTEM_CACHE_STATS_STORAGE_KEY, JSON.stringify(DEFAULT_CACHE_STATS));
...
export const performClearCache = async (
  ...
  // Simulate rapid async cache clearing
...
export const downloadBackupCsv = (
  ...
  // Convert selected entities to friendly readable CSV text
...
// ============================================================
// IMPORT DATA & SCHEMA VALIDATION (Safe Non-destructive)
// ============================================================
...
export const executeImportRestore = (
  ...
  try {
    ...
    // Dispatch global events to refresh all components
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('batutv_articles_updated'));
      window.dispatchEvent(new CustomEvent('batutv_videos_updated'));
      window.dispatchEvent(new CustomEvent('batutv_categories_updated'));
...
export const runSystemHealthDiagnostics = (): SystemHealthReport => {
  ...
  try {
    ...
    totalBytes = 1024 * 500; // 500 KB fallback
  }

  const storageUsedMB = Math.max(0.2, Number((totalBytes / (1024 * 1024)).toFixed(2)));
  const storageQuotaMB = 10.0; // Standard 10MB client storage
