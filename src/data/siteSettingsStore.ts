...
export const INITIAL_SITE_SETTINGS: SiteSettings = {
  ...
  typography: {
    headingFont: 'Outfit' as any, // Supported fonts
    bodyFont: 'Plus Jakarta Sans' as any,
    headingWeight: '800',
    bodyWeight: '400',
...
/**
 * Get current Site Settings with safe fallback
 */
...
/**
 * Save Site Settings & notify subscribers across the frontend
 */
export function saveSiteSettings(
  ...
  try {
    ...
    // Dispatch custom reactive event for instant live-sync
    if (typeof window !== 'undefined') {
      ...
      // Immediately apply to current document head, favicon, CSS variables
      applySiteSettingsToDOM(updated);
    }

    return { success: true, data: updated };
  } catch (err) {
    console.error('Error saving site settings:', err);
    return { success: false, error: 'Gagal menyimpan pengaturan situs ke penyimpanan lokal.' };
  }
}
...
export function applySiteSettingsToDOM(settings: SiteSettings): void {
  ...
  try {
    // 1. Document Title (fallback when on general pages)
    ...
    if (settings.typography) {
      ...
      if (settings.typography.fontSizeScale) {
        ...
        root.style.setProperty('--font-base-scale', scaleMap[settings.typography.fontSizeScale] || '110%');
        root.style.fontSize = scaleMap[settings.typography.fontSizeScale] || '110%';
      }
    }
...
/**
 * Generate Schema.org structured data (NewsMediaOrganization, WebSite)
 */
