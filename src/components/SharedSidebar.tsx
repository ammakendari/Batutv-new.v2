    ...
    >
      ...
      {showPopular && popularNews && popularNews.length > 0 && (
        <div
          ...
          className="popular-widget bg-white rounded-xl border border-slate-200/80 p-4 sm:p-5 shadow-2xs"
        >
          {/* Section Header */}
          ...
          <div className="divide-y divide-slate-100">
            {popularNews.slice(0, 4).map((item) => (
              <article
                ...
                className="popular-item py-3.5 first:pt-0 last:pb-0 group"
              >
                <a
                  href={item.href}
                  onClick={(e) => handlePopularClick(e, item)}
                  aria-label={`Berita terpopuler peringkat ${item.rank}: ${item.title}`}
                  className="flex items-start gap-3.5 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600 rounded-md p-1 -m-1"
                >
                  {/* Red Circular Rank Badge */}
                  <div className="flex-shrink-0 mt-0.5">
                    <span className="w-8 h-8 rounded-full bg-[#c8102e] text-white font-extrabold text-sm flex items-center justify-center shadow-2xs group-hover:scale-105 transition-transform">
                      {item.rank}
                    </span>
                  </div>
      ...
      {showSpecialEvent && specialEvent && (
        <div
          ...
          className="bg-[#0b1320] text-white rounded-xl p-5 sm:p-6 border border-slate-800 shadow-md relative overflow-hidden group"
        >
          ...
          <div className="relative z-10 space-y-4">
            ...
            <a
              ...
              className="inline-flex items-center justify-center w-full px-4 py-2.5 rounded-full border-2 border-white text-white hover:bg-white hover:text-slate-950 font-black text-xs uppercase tracking-wider transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600"
            >
      ...
      {/* 3. TRENDING WIDGET */}
      {showTrending && (
        <div id="sidebar-trending-widget" className="trending-widget bg-white rounded-xl border border-slate-200/80 p-4 sm:p-5 shadow-2xs">
          {/* Section Header */}
          <div className="mb-3.5 sm:mb-4">
            <h3
            ...
            {trendingItems.map((item, idx) => (
              ...
              >
                ...
                >
                  ...
                  <div className="w-18 h-18 sm:w-20 sm:h-20 flex-shrink-0 rounded-lg overflow-hidden bg-slate-100 border border-slate-200/60 relative">
                    <img
                      src={item.imageUrl}
                      alt={item.title}
          ...
          {showMoreTrendingButton && (
            <div className="pt-4 mt-2 border-t border-slate-100">
              <a
                ...
                className="w-full py-2.5 px-4 bg-[#8b181b] hover:bg-[#a31a1e] text-white font-black text-xs tracking-wider uppercase rounded-lg flex items-center justify-center gap-1.5 transition-colors shadow-2xs focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600"
              >
      ...
      {showViralTopics && viralTopics && viralTopics.length > 0 && (
        <div id="sidebar-viral-widget" className="viral-widget bg-white rounded-xl border border-slate-200/80 p-4 sm:p-5 shadow-2xs">
          {/* Section Header */}
          <div className="mb-3.5 sm:mb-4">
            <h3
            ...
            {viralTopics.map((topic, idx) => (
              ...
              >
                ...
                >
                  ...
                  <div className="w-9 h-9 sm:w-10 sm:h-10 flex-shrink-0 bg-[#1e2329] text-white rounded-lg flex items-center justify-center font-black text-sm sm:text-base font-sans tracking-tighter shadow-2xs group-hover:bg-[#c8102e] transition-colors">
                    #{topic.rank}
                  </div>
