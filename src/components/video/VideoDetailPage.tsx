...
export const VideoDetailPage: React.FC<VideoDetailPageProps> = ({
  ...
  const shareTitle = `${currentVideo.title} ${settings.seo.titleSeparator || '|'} Video ${siteName}`;
  ...
  const handleShareTwitter = () => {
    const url = `https://twitter.com/intent/tweet?url=${encodeURIComponent(canonicalUrl)}&text=${encodeURIComponent(shareTitle)}`;
    window.open(url, '_blank', 'noopener,noreferrer,width=600,height=400');
  };

  const handleShareWhatsApp = () => {
    const url = `https://api.whatsapp.com/send?text=${encodeURIComponent(`${shareTitle} - ${canonicalUrl}`)}`;
    window.open(url, '_blank', 'noopener,noreferrer');
  };

  const handleShareTelegram = () => {
    const url = `https://t.me/share/url?url=${encodeURIComponent(canonicalUrl)}&text=${encodeURIComponent(shareTitle)}`;
    window.open(url, '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    ...
    >
      <div className="video-layout max-w-[980px] mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-start">
        ...
        {/* ============================================================ */}
        <article className="video-page lg:col-span-8 w-full bg-white rounded-xl border border-slate-200/80 shadow-xs p-4 sm:p-5 md:p-6">
          {/* 1. BREADCRUMB */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-6 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2 self-start sm:self-center">
              ...
              <button
                ...
                aria-label="Salin Tautan Video"
                title={copySuccess ? 'URL Tersalin!' : 'Salin URL Video'}
              >
                {copySuccess ? (
          ...
          {/* 10. TAGS SECTION */}
          <div className="mb-4 p-4 sm:p-5 bg-[#f8f9fa] rounded-lg border border-slate-200/80">
          ...
          {/* 11. SHARE LINK */}
          <div className="mb-8 p-3.5 sm:p-4 bg-[#f8f9fa] rounded-lg border border-slate-200/80 flex items-center justify-between gap-3 sm:gap-4 flex-wrap sm:flex-nowrap">
            ...
            <div className="flex items-center gap-2 sm:gap-2.5 ml-auto sm:ml-0">
              ...
              <button
                ...
                title={copySuccess ? 'Tautan Tersalin!' : 'Salin Tautan'}
              >
                {copySuccess ? (
                  <Check className="w-4 h-4" />
                ) : (
