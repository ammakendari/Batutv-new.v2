...
interface HeroHeadlineGridProps {
  ...
  onSelectArticle?: (article: NewsArticle | HeadlineArticleData) => void;
}
 ...
 *   3. Kolom Iklan Banner Sponsoring di sisi kanan (4 / 12 kolom).
 */
export const HeroHeadlineGrid: React.FC<HeroHeadlineGridProps> = ({
  data = defaultHeroHeadlineData,
  onSelectArticle,
}) => {
  const { main, subHeadlines = defaultHeroHeadlineData.subHeadlines, adBanner = defaultHeroHeadlineData.adBanner } = data;
    ...
    >
      <div className="hero-headline-container max-w-[980px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-start">
          ...
          {/* ========================================================= */}
          {/* RIGHT COLUMN: AD BANNER (Hidden on Mobile/Tablet or Shown at Bottom) */}
          {/* ========================================================= */}
          <div className="lg:col-span-4 w-full">
            <aside
              id="s03-right-ad-banner"
              aria-label="Iklan Sponsor"
              className="w-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col relative"
            >
              {/* Ad Marker Header */}
              <div className="px-3.5 py-1.5 bg-slate-50 border-b border-slate-100 flex items-center justify-between text-[10px] text-slate-400">
                <span className="font-semibold uppercase tracking-wider">Sponsor Ad</span>
              ...
              {/* Ad Card Content */}
              <div className="p-4 sm:p-5 flex flex-col text-center items-center justify-between min-h-[420px] bg-gradient-to-b from-white via-slate-50/50 to-red-50/20">
                
                {/* Brand & Slogan */}
                <div className="w-full flex items-center justify-between pb-3 border-b border-slate-100">
                  <span className="text-[11px] font-bold text-sky-700 tracking-tight">
                    {adBanner?.tagline || 'ENGINEERED FOR good'}
                  </span>
                  <span className="text-base font-black text-blue-900 tracking-wider">
                    {adBanner?.brand || 'EPSON'}
                  </span>
                </div>

                {/* Promo Headline */}
                <div className="my-3 space-y-1">
                  <h3 className="text-base sm:text-lg font-black text-blue-900 uppercase tracking-tight leading-tight">
                    {adBanner?.title || 'MERDEKA MENCETAK ONLINE ROADSHOW.'}
                  </h3>
                  <p className="text-[11px] font-bold text-sky-600">
                    {adBanner?.period || 'Periode promo : 01-31 Agustus 2026'}
                  </p>
                </div>

                {/* Graphic Banner Area */}
                <div className="my-2 w-full relative py-2 flex flex-col items-center">
                  <div className="inline-block bg-gradient-to-r from-red-600 via-red-700 to-red-600 text-white text-xs sm:text-sm font-black px-3.5 py-1 rounded-md shadow-md border-2 border-white transform -rotate-1 mb-2">
                    {adBanner?.promoBadge || 'MERDEKA MENCETAK'}
                  </div>

                  <div className="flex items-center justify-center gap-2 my-2 w-full">
                    <div className="w-24 h-16 bg-slate-900 rounded-lg shadow-sm flex flex-col items-center justify-center text-[9px] text-white font-mono p-1 border border-slate-700">
                      <div className="w-16 h-2 bg-slate-700 rounded-xs mb-1" />
                      <span className="text-red-400 font-bold">L121</span>
                      <span className="text-[8px] text-slate-400">EcoTank</span>
                    </div>
                    <div className="w-28 h-20 bg-slate-900 rounded-lg shadow-md flex flex-col items-center justify-center text-[10px] text-white font-mono p-1.5 border border-slate-700">
                      <div className="w-20 h-2.5 bg-slate-700 rounded-xs mb-1" />
                      <span className="text-red-400 font-bold">L3251 Wi-Fi</span>
                      <span className="text-[8px] text-slate-400">All-in-One</span>
                    </div>
                  </div>
                </div>

                <p className="text-[11px] text-slate-600 leading-relaxed max-w-xs px-1">
                  {adBanner?.description ||
                    'Rayakan Kemerdekaan Indonesia dengan semangat merdeka dalam mencetak, tanpa rasa khawatir, dan berkarya tanpa batas bersama Epson.'}
                </p>

                <div className="w-full mt-3 p-2.5 bg-gradient-to-r from-blue-900 to-sky-800 text-white rounded-xl shadow-sm text-center">
                  <p className="text-xs font-black text-amber-300 uppercase tracking-wide">
                    {adBanner?.voucherText || 'Dapatkan Shopping Voucher (IDR 150.000)'}
                  </p>
                  <p className="text-[10px] text-slate-200 font-medium mt-0.5">
                    {adBanner?.voucherAmount || 'Setiap pembelian Produk L121, L3211, L3251'}
                  </p>
                </div>

                <div className="mt-2.5 flex items-center justify-between w-full text-[9px] text-slate-400">
                  <span>*Syarat dan ketentuan berlaku</span>
                  <a
                    href={adBanner?.targetUrl || '#'}
                    target="_blank"
                    rel="noreferrer"
                    className="text-sky-700 font-bold hover:underline inline-flex items-center gap-0.5"
                  >
                    Beli Sekarang <ExternalLink className="w-2.5 h-2.5" />
                  </a>
                </div>
